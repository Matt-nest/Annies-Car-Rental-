...
- **Payments are handled via Stripe in this codebase — there is NO Square integration.** Despite Square credentials existing operationally, no code here consumes `SQUARE_*` vars and there is no Square SDK dependency. Card-on-file/payments go through Stripe (`cardOnFileService.js`).
