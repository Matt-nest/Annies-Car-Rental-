...
Drive safe — text us if anything comes up. — ${B.name}, ${B.phone}`,
  ...
  delivery_offer: {
    name: 'Delivery Offer', channel: 'sms', trigger: 'automated',
    desc: 'Sent after booking approval if the customer chose self-pickup.',
    subject: null, body: null,
    sms: `Hi {{first_name}}, prefer not to come to us? We can deliver the {{vehicle}} to you and pick it up when you're done.
...
Hope to see you again soon! — ${B.name}`,
  ...
  rental_completed: {
    ...
    desc: 'Sent after the return is confirmed.',
...
— ${B.name}`,
  ...
  late_return_warning: {
    ...
    desc: 'Sent ~1 hour after the scheduled return time.',
    subject: null, body: null,
...
— ${B.name}, ${B.phone}`,
  ...
  late_return_escalation: {
    ...
    desc: 'Sent ~4 hours after the scheduled return time.',
...
${SIGN}`,
  ...
  inspection_complete: {
    name: 'Inspection Complete', channel: 'email', trigger: 'automated',
    desc: 'Sent when the post-return inspection is complete.',
    subject: 'Inspection complete — {{booking_code}}',
    body: `Hi {{first_name}},
...
Your \${{deposit_amount}} deposit is being processed — you'll get a separate note once settlement is finalized. Questions? Call or text ${B.phone}.
...
${SIGN}`,
  ...
  invoice_sent: {
    name: 'Invoice Sent', channel: 'email', trigger: 'automated',
    desc: 'Sent when an invoice is generated.',
    subject: 'Your invoice — {{booking_code}}',
    body: `Hi {{first_name}},
...
View the full breakdown — and dispute any charge — in your portal. We review disputes within 24 hours. Questions? Call or text ${B.phone}.
...
Expect it within 5–10 business days. — ${B.name}`,
  ...
  refund_processed: {
    name: 'Refund Processed', channel: 'email', trigger: 'automated',
    desc: 'Sent when a refund is processed.',
    subject: 'Refund processed — ${{refund_amount}} ({{booking_code}})',
    body: `Hi {{first_name}},

A refund of \${{refund_amount}} has been processed for {{booking_code}} to your original payment method.

Expect it within 3–5 business days. If you don't see it within five, call or text ${B.phone}.
