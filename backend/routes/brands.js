...
router.get('/:id/env', requireAuth, requireRole('owner'), asyncHandler(async (req, res) => {
  ...
  const lines = [
    ...
    `PAYMENT_PROVIDER=square # Annie=square, JD Coastal=stripe`,
    `VITE_PAYMENT_PROVIDER=square # Must match PAYMENT_PROVIDER`,
    ``,
    `# Square (Annie only)`,
    ...
    `SQUARE_WEBHOOK_URL=https://${b.domain || 'example.com'}/api/v1/square/webhook`,
    `VITE_SQUARE_APPLICATION_ID=`,
    `VITE_SQUARE_LOCATION_ID=`,
    `VITE_SQUARE_ENVIRONMENT=production`,
    ``,
    `# Stripe (JD Coastal only)`,
