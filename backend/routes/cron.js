...
/**
 * GET /cron/auto-expire
 * Runs every 6 hours
 * - 48h+ unapproved → auto-decline
 * - 24-48h unapproved → remind Annie
 */
router.get('/auto-expire', async (req, res) => {
  const results = { declined: 0, reminded: 0 };
  ...
  try {
    ...
    console.log('[CRON/auto-expire]', results);
    res.json({ ok: true, ...results });
  } catch (err) {
