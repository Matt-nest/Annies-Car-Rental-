...
/**
 * Create a Square payment for a customer extension + record the transaction.
 */
export async function createExtensionSquarePayment(bookingId, newReturnDate, { source_id, expectedTotalCents, idempotencyKey } = {}) {
  ...
  try {
    ...
    console.warn('[Extension] could not record pending row:', e.message);
  }
  ...
  try {
    const res = await squareRequest('/v2/payments', {
      ...
      body: {
        ...
        note: `${brand.squareDescriptionPrefix || 'Rental'} — ${booking.booking_code} extension: +${quote.additionalDays} days (${vehicleName})`,
      },
    });
    squarePayment = res.payment;
  } catch (err) {
    if (extensionId) {
      await supabase
        .from('rental_extensions')
  ...
  if (extensionId && squarePayment) {
    await supabase
      .from('rental_extensions')
      .update({ payment_intent_id: squarePayment.id, status: 'paid', paid_at: new Date().toISOString() })
      .eq('id', extensionId)
      .then(() => {}, () => {});
  }

  const applyResult = await _applyExtensionToBooking(booking, {
    ...
    actor: 'customer',
    payment: { method: 'square', referenceId: squarePayment.id, status: 'completed' },
    matchExtensionBy: extensionId ? { id: extensionId } : null,
  });
  ...
  return {
    ...
    amount: squarePayment.amount_money.amount,
    extensionId,
    quote: publicQuote,
    ...applyResult,
  };
...
/** Mark a pending extension failed. */
export async function markExtensionFailed(paymentId) {
  await supabase
    .from('rental_extensions')
    .update({ status: 'failed' })
