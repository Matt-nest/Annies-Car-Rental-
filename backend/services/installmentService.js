...
/**
 * Record a succeeded installment payment — writes a 'rental' payment and
 * marks the installment paid.
 */
export async function confirmInstallmentPayment(paymentId, bookingId, installmentId, planId, amountCents, bookingCode) {
  const { data: existing } = await supabase
    .from('payments').select('id').eq('reference_id', paymentId).maybeSingle();
  ...
  if (installmentId) {
    await supabase
      .from('payment_installments')
      .update({ status: 'paid', paid_at: new Date().toISOString(), payment_intent_id: paymentId, last_error: null })
      .eq('id', installmentId);
  ...
  createNotification(
    ...
    `Booking ${bookingCode || ''} — installment charged to card on file`,
    `/bookings/${bookingId}`,
  ...
  try {
    const res = await squareRequest('/v2/payments', {
      ...
      body: {
        ...
        note: `${brand.squareDescriptionPrefix || 'Rental'} — ${booking.booking_code} installment ${inst.sequence}`,
      },
    });

    const payment = res.payment;
    if (['COMPLETED', 'APPROVED'].includes(payment.status)) {
      await confirmInstallmentPayment(
        payment.id,
        booking.id,
        inst.id,
        inst.plan_id,
      ...
      return { id: inst.id, status: 'paid', pi_id: payment.id };
    }

    await supabase.from('payment_installments')
      .update({ status: 'failed', payment_intent_id: payment.id, last_error: `status:${payment.status}` })
      .eq('id', inst.id);
    return { id: inst.id, status: 'failed', reason: payment.status };
  } catch (err) {
