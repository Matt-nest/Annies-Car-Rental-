...
import { Users, Fuel, Gauge, Settings2, CheckCircle2, XCircle, MapPin, ArrowLeft, Phone, Sun, Moon, Star, ArrowRight, X } from 'lucide-react';
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <AnimatePresence mode="wait">
              <motion.div key={theme} initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: DURATION.fast, ease: EASE.smooth }}>
                        ...
                        <div className="flex gap-0.5 mt-2">
                          {[1, 2, 3, 4, 5].map((i) => (
                            <Star
                              ...
                              size={14}
                              fill={i <= review.rating ? 'var(--accent-color)' : 'none'}
                              stroke={i <= review.rating ? 'var(--accent-color)' : 'var(--text-tertiary)'}
                              strokeWidth={1.5}
                            />
                          ))}
                        </div>
