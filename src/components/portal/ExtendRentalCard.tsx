...
function ExtensionPaymentForm({
  ...
  const handlePay = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_URL}/portal/extension/confirm`, {
        method: 'POST',
      ...
      // Payment succeeded but our confirm call failed — the Stripe webhook will
      // reconcile it. Surface a soft message rather than a hard failure.
      setError(
        (err?.message || 'Something went wrong') +
        ' — your payment went through and we\'ll update your dates shortly. Refresh in a moment.'
      );
      setPaying(false);
    }
  };
...
export default function ExtendRentalCard({
  ...
  const handleProceedToPayment = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`${API_URL}/portal/extension/create-payment`, {
        method: 'POST',
        headers: authHeaders,
        body: JSON.stringify({ newReturnDate: quote.newReturnDate, expectedTotalCents: quote.amountCents }),
      });
      const data = await res.json();
      ...
      <div className="p-5 space-y-4">
        ...
        {step === 'done' ? (
          ...
          <>
            ...
            {step === 'paying' && clientSecret && quote && (
              <Elements
              ...
              >
                <ExtensionPaymentForm
                  ...
                  token={token}
                  onPaid={handlePaid}
                  onCancel={() => { setStep('quoted'); setClientSecret(null); }}
                />
              </Elements>
            )}
          </>
